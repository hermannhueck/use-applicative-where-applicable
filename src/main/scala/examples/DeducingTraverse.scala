...
object DeducingTraverse extends App {
  ...
  val res18 = listSequence1(
    List(Option("---"), Option("List[Maybe[A]]"), Option("becomes"), Option("Maybe[List[A]]"), Option("---"))
  )
  // res18: Option[List[String]] = Some(List(---, List[Maybe[A]], becomes, Maybe[List[A]], ---))
  println(res18)

  val res19 = listSequence2(
    List(Option("---"), Option("List[Maybe[A]]"), Option("becomes"), Option("Maybe[List[A]]"), Option("---"))
  )
  // res19: Option[List[String]] = Some(List(---, List[Maybe[A]], becomes, Maybe[List[A]], ---))
  println(res19)

  val res20 = listSequence2(
    List(Option("---"), Option("List[Maybe[A]]"), Option("becomes"), Option("Maybe[List[A]]"), Option.empty)
  )
  ...
  val res24 =
    listTraverse1[Option, String, String](List("---", "List[Maybe[A]]", "becomes", "Maybe[List[A]]", "---"))(s =>
      Option(s.toUpperCase)
    )
  // res24: Option[List[String]] = Some(List(---, LIST[MAYBE[A]], BECOMES, MAYBE[LIST[A]], ---))
  println(res24)

  val res25 =
    listTraverse2[Option, String, String](List("---", "List[Maybe[A]]", "becomes", "Maybe[List[A]]", "---"))(s =>
      Option(s.toUpperCase)
    )
